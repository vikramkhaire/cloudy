...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 19 02:56:21 2021
...
#def log_prior(theta, interp_logf_for_upper_limit, data_col_upper_limit):
def log_prior(theta, interp_logf_for_upper_limit):
    lognH, logZ =  theta
    ...
    np.save(os.path.join(outpath, 'upper_limit_col'), upper_limit_col)
...
#    condition_array_upper = condition_upper.all()
    ...
    # flat prior
#    if -6 < lognH < -2 and -3 < logZ < 1 and condition_array_upper == True:
#            return 0.0
    # flat prior
...
#def log_posterior(theta, interp_func, interp_func_for_upper_limit, data_col_upper_limit,
#                  data_col, sigma_col):
#    log_p = log_prior(theta, interp_logf_for_upper_limit = interp_func_for_upper_limit,
...
def log_posterior(theta, interp_func, interp_func_for_upper_limit,
                  data_col, sigma_col):
    log_p = log_prior(theta, interp_logf_for_upper_limit = interp_func_for_upper_limit) + \
            log_likelihood(theta, interp_logf = interp_func, obs_ion_col = data_col, col_err = sigma_col)
...
def run_mcmc(model_path, Q_uvb, ions_to_use, ions_to_use_for_lower_limit = None, 
             ions_to_use_for_upper_limit = None, data_col_lower_limit=None, data_col_upper_limit=None,
             data_col=None, true_Q =18, uvb = 'KS18', figname = 'testT.pdf', same_error = False):
...
#    print('Ion Name: ', ionname)
    ...
    interp_logf = get_interp_func(model_path = model_path, ions_to_use = ions_to_use, 
                                  Q_uvb = Q_uvb, uvb = uvb)
    interp_logf_for_upper_limit = get_interp_func(model_path = model_path, 
                                                  ions_to_use = ions_to_use_for_upper_limit, 
                                                  Q_uvb = Q_uvb, uvb = uvb)
...
#                                    args=(interp_logf, interp_logf_for_upper_limit, data_col_upper_limit,
#                                          data_col, sigma_col))
    sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior,
                                    args=(interp_logf, interp_logf_for_upper_limit,
                                          data_col, sigma_col))
...
ions_to_use_for_upper_limit = ['N+4', 'S+', 'S+2', 'S+3', 'S+5', 'Fe+', 'N+2']
#data_col_upper_limit = np.array([13.4, 14.4, 14.0, 13.7, 12.8, 11.4, 18])
#data_col_upper_limit = np.array([19, 16.8, 18.4, 19.5, 18.5, 15.5])
#ions_to_use_for_lower_limit = ['C+2', 'Si+2']
#data_col_lower_limit = np.array([11.5, 11.0])
...
#                                  ions_to_use_for_upper_limit=ions_to_use_for_upper_limit,
#                                  data_col=data_col, data_col_upper_limit=data_col_upper_limit,
