...
def run_parallel(logNHI,logZ, uvb_Q, uvb,logT,z_re):
    ...
    check_fits= cloudy_run_path_and_file[1].split('.in')[0] + '.fits'
    if os.path.isfile(check_fits)==False:
        # write input file and run cloudy
        ions, params    =   cloudy_params_defaults(uvb = uvb, uvb_Q=uvb_Q, log_hden=[-6.0, -2.0, 0.02], stop_NHI = logNHI, metal = logZ,T = 10**logT,sequential = True,z=z_re,CMB='CMB',remove_dot_out_file = True)
...
logNHI_array        =   np.array([15.11, 15.02, 16.05, 14.48, 14.69, 15.25, 15.61, 15.89, 15.28,
